...
영상 후반부에서 향후 5년의 변화로 **Physical AI**가 언급된다. 지금은 AI 에이전트가 데이터와 문서를 조회하고 분석해 인사이트를 제공하는 소프트웨어 레벨이지만, 앞으로는 공장 라인의 파라미터를 자동으로 조절하고 로봇과 결합해 물리적 작업까지 지원하는 방향으로 확장될 것이라는 전망이다.
